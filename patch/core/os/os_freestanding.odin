...
open :: proc(path: string, mode: int = O_RDONLY, perm: int = 0) -> (Handle, Error) {
	unimplemented("core:os procedure not supported on freestanding target")
}
...
exit :: proc "contextless" (code: int) -> ! {
	unimplemented_contextless("core:os procedure not supported on freestanding target")
}
